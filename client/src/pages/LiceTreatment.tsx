...
import { ShieldCheck, Phone, CheckCircle, Lock, MapPin, Clock, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function LiceTreatment() {
  ...
  const nextImage = () => {
    setCurrentImageIndex((prev) => (prev + 1) % liceTreatmentImages.length);
  };

  const previousImage = () => {
    setCurrentImageIndex((prev) => (prev - 1 + liceTreatmentImages.length) % liceTreatmentImages.length);
  };
                  ...
                  {liceTreatmentImages.length > 1 && (
                    <>
                      ...
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                        {liceTreatmentImages.map((_, index) => (
                          <button
                            key={index}
